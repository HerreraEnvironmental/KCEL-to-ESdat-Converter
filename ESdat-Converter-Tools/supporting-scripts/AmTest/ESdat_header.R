...
## Import files
  ...
  files         <- list.files("data/AmTest/data_raw", pattern = "*.csv")
  lab_reports   <- substring(files, 1, 8) # assumes report number is in file name, adjust character numbers as needed
  ...
  files         <- list.files("data/AmTest/data_secondary")
  ...
  for (i in 1:length(lab_reports)){
    ...
    xmlAttrs(report_node) = c(Lab_Signatory = "Aaron Young",                
                              Lab_Name = "AmTest",
    ...
    saveXML(doc, file = paste0("data/AmTest/data_secondary/", proj_num, ".", report_num, ".ESdatHeader.xml"))
