...
## Purpose of script:   Create ESdat headers from Onsite EDDs
## Author:              N. VandePutte
...
## Import files
  ...
  files         <- list.files("./data/Onsite/data_secondary")
  
  # Build XML based on project
  for (i in 1:length(lab_reports)){
    report_date <- as.character(Sys.Date()) # report date will be the date the header file is generated
    ...
    xmlAttrs(report_node) = c(Lab_Signatory = "David Baumeister",
                              Lab_Name = "OnSite Environmental",
    ...
    saveXML(doc, file = paste0("./data/Onsite/data_secondary/", proj_num, ".", report_num, ".ESdatHeader.xml"))
